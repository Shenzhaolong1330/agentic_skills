...
`SkillContext` 包含 `run_id`、`mode`、`execute`、`hardware_allowed`、operator token 状态、artifact dir、manifest path、robot server、阈值、reset attempt、held object state。`StageResult` 和 `TaskResult` 会写入 trace，失败时也必须写 `task_result.json`。
...
1. `context.mode == live`
2. `context.execute is true`
3. `context.hardware_allowed is true`
4. CLI operator token 与 `AGENTIC_SKILLS_HARDWARE_TOKEN` 匹配
5. manifest entrypoint 允许该用途；reset recovery 还需要 `allowed_as_recovery=true`
...
reset 没有被禁用。`AUTO_RESET_RECOVERY` 在 abnormal robot state 或 reset guard 需要 reset 时触发。mock 返回 `MOCK_RESET_OK`；dry_run 写 reset command plan 并返回 `PLANNED_ONLY`；live gate 通过后才允许 reset。持管阶段 reset 后默认 abort，原因是 reset 可能移动双臂或改变夹爪/持物状态，需要对象状态重验证。
